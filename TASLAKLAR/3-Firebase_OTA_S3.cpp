...
void performOTAUpdate(String url)
{
    Serial.printf("[Info]: Serbest Heap: %d bytes\n", ESP.getFreeHeap());
    Serial.printf("[Info]: Min Heap: %d bytes\n", ESP.getMinFreeHeap());
    Serial.printf("[Info]: PSRAM Kullanılıyor mu: %s\n", psramFound() ? "Evet" : "Hayır");
...
void loop()
{
    delay(60000); // 1 dakikada bir kontrol
    checkForOTAUpdate();
}
